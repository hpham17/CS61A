...
def apply_primitive(procedure, args, env):
    ...
    """
    "*** YOUR CODE HERE ***"
    pythonlist = list(args)
    if procedure.use_env:
        pythonlist.append(env)
    try:
        return procedure.fn(*pythonlist)
    except TypeError:
        raise SchemeError("unexpected arg")
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            ...
            raise SchemeError("unexpected number of args")
        return frame
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(var):
            rest = Pair(target.second, vals.second)
            lambda_expr = Pair("lambda", rest)
            env.define(var, scheme_eval(lambda_expr, env))
            return var
        else: 
            raise SchemeError("bad arg to define")
    else:
...
def do_let_form(vals, env):
    ...
    for var in bindings:
        # print(var[0])
        # print(var[1])
        if len(var) != 2 or not scheme_symbolp(var[0]):
            raise SchemeError("bad number of arguments")
        names = Pair(var[0], names)
        values = Pair(scheme_eval(var[1], new_env), values)

    for i in range(len(names)):
        new_env.define(names[i], values[i])
...
def do_if_form(vals, env):
    ...
    check_form(vals, 2, 3)
    "*** YOUR CODE HERE ***"
    exprs = scheme_eval(vals[0], env)
    if scheme_false(exprs):
        if len(vals) is 2:
            return okay
        return vals[2]
    elif scheme_true(exprs):
        return vals[1]
...
def do_and_form(vals, env):
    ...
    if vals is nil:
        return True
    for expr in vals:
        eval_expr = scheme_eval(expr, env)
        if not scheme_true(eval_expr):
            return False
    return eval_expr
...
def do_or_form(vals, env):
    ...
    for expr in vals:
        eval_expr = scheme_eval(expr, env)
        if scheme_true(eval_expr):
            return quote(eval_expr)
    return eval_expr
...
def check_formals(formals):
    ...
    def checker(formals):
        if formals is not nil:
            ...
            if len(formals) < 2:
                if formals.second is not nil:
                    raise SchemeError("Not a well-formed list")
            else:
                if formals.first in symbols:
                    raise SchemeError("Can't have repeated symbols")
                symbols.append(formals.first)
                return checker(formals.second)
    checker(formals)
