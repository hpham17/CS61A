...
(define (combine pair) (car pair))
(define (combine2 pair) (cadr pair))

(define (zip pairs) 
  
  (if (null? pairs) '(() ())
  (cons (apply-to-all combine pairs) (cons (apply-to-all combine2 pairs) nil)))
)
...
(define (list-partitions total max-pieces max-value)
  (define (partition-helper total max-pieces max-value parts-list)
    (cond ((= total 0) (list parts-list))
      ((or (= max-pieces 0) (= max-value 0)) '()) 
      (else (append (partition-helper (- total max-value) (- max-pieces 1) max-value (append parts-list (list max-value)))
      (partition-helper total max-pieces (- max-value 1) parts-list)))))
   (partition-helper total max-pieces max-value '()))
...
(define (analyze expr)
  (cond ((atom? expr) expr)
        ((quoted? expr) (cons 'quote (cdr expr)))
        ((or (lambda? expr)
             (define? expr))
         (let ((form   (car expr))
               (params (cadr expr))
        ...
        ((let? expr)
          (let ((values (cadr expr))
              (body   (cddr expr)))
              (cons (cons 'lambda (cons (car (zip values)) (append (apply-to-all analyze body) nil))) (apply-to-all analyze (cadr (zip values))))
          )
        )
